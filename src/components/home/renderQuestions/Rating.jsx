...
export const Rating = ({ postId }) => {
   ...
   const doLike = async () =>{
      if(!canRate()) return;
      if(postObject.likedBy.find(id => id === user.id)) return;
      
      if(postObject.dislikedBy.find(id => id === user.id)){
         postObject.dislikedBy = postObject.dislikedBy.filter(id => id !== user.id);
      }

      postObject.likedBy.push(user.id);
      await editPost(postObject);
   }

   const doDislike = async () =>{
      if(!canRate()) return;
      if(postObject.dislikedBy.find(id => id === user.id)) return;
      
      if(postObject.likedBy.find(id => id === user.id)){
         postObject.likedBy = postObject.likedBy.filter(id => id !== user.id);
      }

      postObject.dislikedBy.push(user.id);
      await editPost(postObject);
   }
